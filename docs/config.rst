...
.. envvar:: REACT_APP_CHECK_AUTH_EXPIRY_INTERVAL_MS
    ...
    the number of milliseconds to sleep between each check.
    This number is also used to preemptively trigger a refresh. Meaning, if the access token
    hasn't yet expired but it will *in* ``REACT_APP_CHECK_AUTH_EXPIRY_INTERVAL_MS`` milliseconds,
    that triggers a authentication refresh too.
